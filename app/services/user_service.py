...
class UserService:
    ...
    async def upload_profile_picture(cls, session: AsyncSession, user_id: UUID, file, filename: str) -> Optional[User]:
        """
        Upload a user's profile picture to Minio and update their profile_picture_url in the database.
        
        :param session: AsyncSession for DB operations
        :param user_id: UUID of the user
        :param file: File-like object containing the image data (e.g., BytesIO)
        :param filename: Original filename of the uploaded image
        :return: Updated User object or None if error occurred
        """
        user = await cls.get_by_id(session, user_id)
        if not user:
        ...
        if ext not in allowed_exts:
            logger.error("Invalid image format. Allowed: JPEG, PNG.")
            return None

        # Create object name
        object_name = f"profile_{user_id}.{ext}"
        ...
        # Construct URL
        file_url = f"{MINIO_PUBLIC_ENDPOINT}/{MINIO_BUCKET_NAME}/{object_name}"

        # Update user record
        user.profile_picture_url = file_url
        session.add(user)
        await session.commit()
        logger.info(f"User {user_id} profile picture uploaded successfully.")
        return user
